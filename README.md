...
### 形态学滤波
#### 膨胀与腐蚀
它们都是对图像的高亮部分进行处理，膨胀就是对高亮部分进行膨胀，使效果图拥有比原图更大的高亮区域，腐蚀反之。     
...
### 仿射变换
指在几何中，一个向量空间进行一次线性变换并加上一个平移，变换到另一个向量空间的过程。      
任何一个仿射变换都可以数学表示为乘以一个矩阵（线性变换）再加上一个向量（平移）。      
仿射变换可以表示这三种常见的变换形式：旋转（线性变换）、缩放（线性变换）、平移（向量加）。         
通常可以用2*3矩阵来表示仿射变换，OpenCV中可以通过getAffineTransform求得矩阵，前提是要取图像中任意三点和变换后对应的三点的坐标，根据这些坐标求得矩阵，再用warpAffine应用这个矩阵进行变换。      
也可以通过getRotationMatrix2D这个方法求得二维旋转变换的矩阵。   
### 直方图均衡化
它是灰度变换一个重要应用，广泛用于图像增强。图像的像素灰度变化是随机的，直方图的图形高低不齐，直方图均衡化就是通过拉伸像素强度分布范围来增强图像的对比度，本质是扩大量化间隔，各个灰度区域有明显的边界。                 
